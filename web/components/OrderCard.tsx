...
function formatDate(iso: string): string {
  // Deterministic formatting (avoids server/client locale mismatch).
...
export function OrderCard({ order }: { order: Order }) {
  const c = accentClasses[order.accent];
  return (
    <Card className="group flex flex-col overflow-hidden hover:-translate-y-1 hover:shadow-lg">
      ...
      <div className="flex flex-1 flex-col gap-4 p-5">
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <div className="mt-1.5">
              <Badge color={order.accent} withDot>
                {order.ticker}
              </Badge>
            </div>
          </div>
          <time className="shrink-0 text-xs text-slate-400" dateTime={order.filedAt}>
        ...
        <dl className="grid grid-cols-2 gap-4">
          ...
          <Field label="Duration">{order.duration}</Field>
          <Field label="Target industry">
            <Badge color={order.accent}>{order.targetIndustry}</Badge>
          </Field>
          <Field label="Filed">{formatDate(order.filedAt)}</Field>
        </dl>
        ...
        {/* Evidence footer — every order links back to its source filing */}
